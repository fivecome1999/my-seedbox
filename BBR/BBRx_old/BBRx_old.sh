...
if dkms status | grep -q "bbrx_old/"; then
	for module_ver in $(dkms status | grep "bbrx_old/" | awk -F, '{print $1}' | awk -F/ '{print $2}' | sort -u); do
		echo "Removing existing bbrx_old module version: $module_ver"
		dkms remove -m bbrx_old -v "$module_ver" --all
	done
fi
...
case "$ID:${VERSION_ID%%.*}" in
    debian:12)
        bbrx_old_source_url="https://raw.githubusercontent.com/fivecome1999/my-seedbox/main/BBR/BBRx_old/tcp_bbrx_old.c"
        ;;
    debian:13)
        bbrx_old_source_url="https://raw.githubusercontent.com/fivecome1999/my-seedbox/main/BBR/BBRx_old/tcp_bbrx_old_debian13.c"
        ;;
    *)
...
wget -O $HOME/tcp_bbrx_old.c "$bbrx_old_source_url"
if [ ! -f $HOME/tcp_bbrx_old.c ]; then
	echo "Error: Download failed! Exiting." >&2
	exit 1
fi
# DKMS 模块版本（与内核无关）。建议固定或使用日期字符串
module_ver=1.0.0
# algo: 用作 DKMS 包名与内核模块文件名（tcp_bbrx_old.ko），保持带下划线。
algo=bbrx_old
# ca_name: 模块向内核注册的拥塞控制算法名（见 tcp_bbrx_old.c 的 .name 字段）。
# 必须与源码里的 .name 完全一致，且不能与 bbrx/bbrz 撞名，故用 bbrxold。
# sysctl 切换拥塞控制时用的是这个名字，而不是 algo。
ca_name=bbrxold
...
if [ ! $? -eq 0 ]; then
    sed -i '/tcp_bbrx_old/d' /etc/modules
...
net.ipv4.tcp_congestion_control = $ca_name
EOF
...
rm /root/BBRx_old.sh > /dev/null 2>&1
echo "BBR ($ca_name) 安装完成，已即时生效。系统将在 1 分钟后自动重启一次完成收尾。"
shutdown -r +1
